...
def process_apk(apk_file, sha256, features_out_dir, logger):
    ...
    try:
        ...
        if features_out_dir is not None:
            filename_data = os.path.join(features_out_dir, sha256 + ".json")
        ...
        logger.debug(e)
        logger.debug(f"{apk_file} processing failed in {final_time - start_time}s...")
        return None
    else:
        final_time = time.time()
        logger.debug(f"{apk_file} processed successfully in {final_time - start_time}s")
        return [
            f"{k}::{v}"
